...
root_dir = "/workspace/DF2K"+str(args.resolution)+"x"

trainset = DF2KDataset(root_dir, int(args.resolution))
print(len(trainset))
...
model.to(device)
summary(model, input_size=(1, 3, 64, 64), col_names= ("input_size","output_size","num_params","mult_adds"), depth = 4)

scaler = torch.cuda.amp.GradScaler()

batch_size = 432

PATH = 'bsd100_'+str(args.resolution)+'x_y_patches64.pth'
...
counter = 0
while counter < 15:  # loop over the dataset multiple times
    t0 = time.time()
    epoch_psnr = 0
    epoch_loss = 0
    running_loss = 0.0
